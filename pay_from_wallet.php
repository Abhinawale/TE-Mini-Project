...
$em2=mysqli_fetch_assoc($result2);
$wallet_balance=$em2['wallet'];
...
<div class="window">
  <h1>wallet balance:<?php
  while($em1=mysqli_fetch_assoc($result1))
{
    $payable=$em1['total'];
    if($payable<$wallet_balance)
    {
    $balance=$wallet_balance-$payable;
    $update7="UPDATE customer SET wallet='$balance' WHERE mobile='$user_name'";
    $updateresult7=$mysqli->query($update7);
    echo "$balance<br>";
    }
    else{
        echo "LOW BALANCE!!";
