...
{
  $sqla = "SELECT * FROM feedback where city='pune'";
...
{
  $sqla = "SELECT * FROM feedback where city='mumbai'";
  $resulta = $conn->query($sqla);
  $resultb = $conn->query($sqla);
  //$resultc = $conn->query($sqla);
}
...
{
  $sqla = "SELECT * FROM feedback where city='nagpur' ";
  $resulta = $conn->query($sqla);
  $resultb = $conn->query($sqla);
  //$resultc = $conn->query($sqla);
}
...
{
  $sqla = "SELECT * FROM feedback where city='nashik'";
  $resulta = $conn->query($sqla);
  $resultb = $conn->query($sqla);
  $resultc = $conn->query($sqla);
}
...
<?php
echo "<table>";
echo "<tr><th>Mobile Number</th><th>Name</th><th>Complaints</th><th>Action</th>";
echo "</tr>";
...
echo "</td>";
echo "<td>";
echo "{$emp1['complaints']}";
echo "</td>";
echo "<td>";
echo "<form action='complaints_delete.php' method='POST'>";
